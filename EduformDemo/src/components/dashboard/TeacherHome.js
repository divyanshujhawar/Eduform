...
class TeacherHome extends React.Component {
    constructor(props) {
        ...
        const darkVals = ["darkbackground", "darkwords", "darkheader", "Nightform", "#bc6ff1", "#322f3d"];
        ...
        if (cssType === 'dino') {
            ...
            wordDisplay = darkVals[3];
            headerback = darkVals[4];
            headerwordcolor = darkVals[5];
          }
        else {
    ...
    getTeacherCourses() {
        try {
            const response = fetch('/teacher/getCourses/' + UserProfile.getEmail(), {
                method: 'GET',
                headers: { 'Content-Type': 'application/json', 'Accept': 'application/json' },
            })
                .then(res => res.json())
    ...
    bar() {
        ...
        if (this.state.itemPad === 60) {
            this.setState({ itemPad: 175, pageSplit: null });
        }
        else {
            this.setState({ itemPad: 60, pageSplit: <div className="col-md-1 col-sm-0"></div> });
        }
    }
    ...
    showAnnouncements(){
        ...
        past.push(
            <div class="list-group">
                <Announce styler={this.state.pageTheme}/>
            </div>
        );
                ...
                <div style={{ paddingLeft: `${this.state.itemPad}px`, paddingRight: '5%' }} className="flex-container">
                    ...
                    <div style={{ marginTop: '30px' }} className="row">
                        ...
                        <div style={{ paddingRight: '20%', textAlign: 'left' }} className="flex-col-md-8 flex-col-sm-12">
                            ...
                            <h1 className={`${this.state.theWords}`} style={{ fontSize: '2.6rem' }}> Created Assignments</h1>
                            <hr style={{ marginBottom: '30px' }} />
                            <Upcoming styler={this.state.pageTheme}/>
                        </div>
                        {this.state.pageSplit}
                        <div style={{ marginTop: '39px' }} className="flex-col-md-6 flex-col-sm-12">
                            <div style={{marginBottom: '0'}} className="btn-group dropleft">
                                <button style={{ width: '430px', height: '50px', backgroundColor: '#febf63' }} type="button" className="btn dropdown-toggle announce" data-toggle="dropdown" aria-haspopup="true" aria-expanded="true">
                            ...
                            <div>
                                <h1 className={`${this.state.theWords}`} style={{marginTop: '10px',fontSize: '2.6rem' }}> Created Announcements</h1>
                                <hr />
