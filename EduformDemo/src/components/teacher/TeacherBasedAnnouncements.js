        ...
        for (i; i < this.state.announcements.length; i++) {
            ...
            if (((Math.abs(time - passedDay) / milliTime) <= 14)) {  // This checks to make sure time is within 2 week span for announcements
                announceItems.push(
                    ...
                    <a style={{textAlign: 'left',marginBottom: '5px', backgroundColor: '#1089ff' }}  class="item">
                        <a style={{color: '#febf63' }} class="header"><p style={{color: '#febf63',textAlign: 'left'}}>{this.state.announcements[i].courseCode}</p></a>
