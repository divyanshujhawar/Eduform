    ...
    render() {
        ...
        for (i; i < this.state.assignments.length; i++) {
            ...
            if (queryDate === this.state.selectedDate) {
                upcoming.push(
                    <button style={{ color: color1, backgroundColor: `${this.state.primary}`, borderColor: `${this.state.secondary}` }} class="button2">{this.state.assignments[i].filename.substring(0,11)} <br /> <p style={{ fontSize: '.8rem' }}>{this.state.assignments[i].courseCode}</p></button>
                );
            }
        }
