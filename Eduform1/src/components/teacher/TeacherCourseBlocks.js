...
class TeacherCourseBlocks extends React.Component {
    ...
    constructor(props) {
        ...
        this.state = {
            ...
            //teacherCourses: [{"courseName" : "SE", "courseCode" : "P456"},{"courseName" : "SE", "courseCode" : "P456"}
            //,{"courseName" : "SE", "courseCode" : "P456"},{"courseName" : "SE", "courseCode" : "P456"}],

            teacherCourses: CourseProfile.getCourseProfile(),
    ...
    render() {
        ...
        let i = 0;
        for (i; i < this.state.teacherCourses.length; i++) {
            courseItems.push(
                <div style={{ paddingLeft: '40px', marginBottom: "20px" }} className="col-md-4 col-sm-10">
                    <Link to={{                                                                                       // I added this.state in front of teacherCourses, change back in integration
                        pathname: '/teacher-course/' + this.state.teacherCourses[i].courseCode, state: {
                            currentClass: this.state.teacherCourses[i].courseCode
                        }
                    }}>
                        <a style={{ fontFamily: `${this.state.theBackground}`, backgroundColor: `${this.state.selectheaderback}` }} class="ui card">
                            <div class="content">
                                <div style={{ fontFamily: `${this.state.theBackground}`, color: `${this.state.alternate}` }} class="header">{this.state.teacherCourses[i].courseName}</div>
                                <div class="meta">
                                    <span style={{ fontFamily: `${this.state.theBackground}`, color: 'black' }} class="category">{this.state.teacherCourses[i].courseCode}</span>
                                </div>
