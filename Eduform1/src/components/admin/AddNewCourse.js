...
class AddNewCourse extends Component {
    ...
    async addCourse(event){
        ...
        this.setState({courseDetails: document.getElementById("courseDetails").value});

        this.state.courseRequest.courseCode = this.state.courseCode;
        this.state.courseRequest.courseName = this.state.courseName;
        this.state.courseRequest.courseDetails = this.state.courseDetails;
    ...
    handleSubmit(event) {
        ...
        let vars = ["courseCode","courseName","courseDetails"];
                    ...
                    <div style={{paddingRight: '2%', paddingLeft: '2%' }} className="row myIntro">
                        <div style={{width: '500px',marginTop: '16px',padding: '0% 3% 1% 3%', paddingTop: '2%',paddingBottom: '2%' }} className="flex-col-sm-12 flex-col-md-12 bg-black myLogIn text-primarys">
                            ...
                            <form id="course-form" onSubmit={this.handleSubmit} noValidate>
                                ...
                                <div className="form-group">
                                    <textarea type="text" id="courseDetails" name="courseDetails" className="form-control" placeholder="Enter course details" style={{height: '70px',fontSize: '.9rem' }} onChange={this.handleChange} noValidate />
                                    {errors.courseDetails.length > 0 && <span className='error'>{errors.courseDetails}</span>}
                                </div>
