...
class GradeBlocks extends React.Component {
    ...
    getCourseAssignments() {
        try {
            const response = fetch('/teacher/getCourseAssignments/' + this.state.course, {
                method: 'GET',
    ...
    render() {
        ...
        let passedUserId = this.props.theUserId;
        ...
        for (i; i < this.state.assignments.length; i++) {
            if (this.state.course === this.state.assignments[i].course_code && passedUserId === this.state.assignments[i].user_id) {
                if (query === "") {
